...
const loadfrontPage = () => { //function to load cards of the users to like or dislike them
    ...
    <div>
        ...
        <div className='cardAndButtons'>
            ...
            <TinderCard key={currentShown} onSwipe={(direction)=>{swiped(direction,result[currentShown])}}preventSwipe={['up', 'down']}><div className='TinderCard'><h3 >{result[currentShown].username }</h3><br/><p> Bio: {result[currentShown].information}</p><br/><p> Member since: {result[currentShown].registerationdate}</p></div></TinderCard>
            
            </div>
            <div className='ButtonsDiv'> 
                <button className='Buttons' onClick={Dislike}>Dislike</button>
                <button className='Buttons' onClick={Like}>Like</button>
                
            </div> 
        </div>
        )}
        {result && result.length === 0 && <p style={{color:"green"}}>No users to display.</p>}

        {result && result.length > 0 && currentShown== result.length && (
        <p className='Text'>You have seen all available users.</p>
        )}
